 ...
 * @author admin
 */
public class CountryEventGenderBar extends javax.swing.JPanel {

    /**
     * Creates new form CountryEventGenderBar
     */
    JPanel mainJPanel;
    CountryComplaint cc;
    String key = "";
    ...
    HashMap male,female;
    JFreeChart chart;
    public CountryEventGenderBar(JPanel mainJPanel,CountryComplaint cc) {
        initComponents();
    ...
    void setValues(String s){
        switch(s.trim()){
                ...
                case "Male":
                    if(male.containsKey(key)){
                        tempInt = (Integer)male.get(key) + 1;
                        male.put(key, tempInt);
                    }
                    else{
                        male.put(key, startInt);
                    }
                    break;
                case "Female":
                    if(female.containsKey(key)){
                        tempInt = (Integer)female.get(key) + 1;
                        female.put(key, tempInt);
                    }
                    else{
                        female.put(key, startInt);
                    }
                    break;
     ...
     */
     private CategoryDataset createDataset(){
         ...
         Integer num;
         for (Object key : male.keySet()) {
		num = (Integer)male.get(key);
             dataset.addValue(num.intValue(), "Male", (String)key);
	 }
         for (Object key : female.keySet()) {
		num = (Integer)female.get(key);
             dataset.addValue(num.intValue(), "Female", (String)key);
	 }
    ...
    protected static BufferedImage draw(JFreeChart chart, int width, int height)
    {
     ...
     private JFreeChart createChart(final CategoryDataset dataset) {
        ...
        chart = ChartFactory.createBarChart(
            "Gender in Hospital",         // chart title
            "Gender-Country",               // domain axis label
    ...
    private void saveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveBtnActionPerformed
        ...
        try{
            String filename = "CountryEventGenderBar" + dateFormat.format(cal.getTime()).toString().trim() + "jpg";
